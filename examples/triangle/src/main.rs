    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::Resized(new_size) => {
                ...
                let new_extent = types::Extent2D {
                    width: new_size.width,
                    height: new_size.height,
                };

                // Resize the swapchain
                if let Err(e) = swapchain.resize(new_extent) {
                    tracing::error!("Failed to resize swapchain: {:?}", e);
                    return;
                }

                // Recreate command pool since swapchain images changed
                self.command_pool = Some(
                    device
                        .create_command_pool(&self.swapchain.clone().unwrap())
                        .unwrap(),
                );
            ...
            WindowEvent::RedrawRequested => {
                ...
                let image = match swapchain.acquire_next_image() {
                    Ok(img) => img,
                    Err(
                        moonfield_rhi::types::RhiError::AcquireImageFailed(msg),
                    ) => {
