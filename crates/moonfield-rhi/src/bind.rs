...
pub(crate) mod vulkan_impl {
    ...
    impl Sampler {
        /// Create an owned sampler from a description; `Drop` destroys it.
        pub fn new(device: &VulkanDevice, desc: &crate::types::SamplerDesc) -> crate::Result<Self> {
            let mipmap_mode = match desc.mipmap_filter {
                Some(crate::types::Filter::Linear) => vk::SamplerMipmapMode::LINEAR,
            ...
            let sampler = unsafe {
                device.raw().create_sampler(&info, None).map_err(|e| {
                    crate::Error::Backend(format!("failed to create sampler: {e:?}"))
                })?
            };
            Ok(Self {
                sampler,
