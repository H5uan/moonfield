...
use super::{VulkanBuffer, VulkanPipeline, VulkanSwapchain};
use crate::{types::*, *};
...
impl CommandBuffer for VulkanCommandBuffer {
    ...
    fn end(&self) -> Result<(), RhiError> {
        unsafe {
            self.device.end_command_buffer(self.command_buffer).map_err(|e| {
                RhiError::InitializationFailed(format!(
    ...
    fn begin_render_pass(
        ...
        unsafe {
            let image_view =
                ash::vk::ImageView::from_raw(image.image_view as u64);
            let swapchain =
                self.swapchain.as_ref().and_then(|w| w.upgrade()).unwrap();
            let images = swapchain.images.read().unwrap();
            let swapchain_image = images[image.index as usize];
