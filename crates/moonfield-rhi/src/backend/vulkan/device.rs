...
use super::{
    VulkanBuffer, VulkanCommandPool, VulkanPipeline, VulkanQueue,
    VulkanShaderModule, VulkanSurface, VulkanSwapchain,
};
use crate::{types::*, *};
...
impl Device for VulkanDevice {
    fn create_swapchain(
        ...
        tracing::debug!(
            "Creating Vulkan swapchain with format: {:?}, extent: {:?}",
            desc.format,
            desc.extent
        );
        ...
        unsafe {
            ...
            tracing::info!(
                "Vulkan swapchain created successfully with {} images",
                image_count
            );
            ...
            Ok(Arc::new(VulkanSwapchain {
                ...
                swapchain_loader,
                surface: vk_surface.surface,
                swapchain: std::sync::RwLock::new(swapchain),
                images: std::sync::RwLock::new(images),
                image_views: std::sync::RwLock::new(image_views),
                format: desc.format,
                extent: std::sync::RwLock::new(desc.extent),
                image_available_semaphores,
                render_finished_semaphores: std::sync::RwLock::new(
                    render_finished_semaphores,
                ),
                queue: self.queue,
                current_frame: std::sync::atomic::AtomicUsize::new(0),
                image_layouts: std::sync::Mutex::new(
    ...
    ) -> Result<Arc<dyn ShaderProgram>, RhiError> {
        tracing::debug!(
            "Creating Vulkan shader module for stage: {:?}",
            desc.stage
        );
        unsafe {
            ...
            let create_info =
                ash::vk::ShaderModuleCreateInfo::default().code(code);
            ...
            tracing::info!(
                "Vulkan shader module created successfully for stage: {:?}",
                desc.stage
            );
    ...
    fn create_pipeline(
        ...
        unsafe {
            ...
            let binding_descriptions: Vec<_> = desc
                .vertex_input
                .bindings
                .iter()
            ...
            let attribute_descriptions: Vec<_> = desc
                .vertex_input
                .attributes
                .iter()
            ...
            let memory_type_index = self
                ...
                .ok_or_else(|| {
                    RhiError::BufferCreationFailed(
                        "Could not find suitable memory type for buffer"
                            .to_string(),
                    )
                })?;
    ...
    fn create_command_pool(
        ...
        unsafe {
            ...
            let command_pool = self
                ...
                .map_err(|e| {
                    RhiError::CommandPoolCreationFailed(format!(
    ...
    fn get_queue(&self) -> Arc<dyn Queue> {
        Arc::new(VulkanQueue { device: self.device.clone(), queue: self.queue })
    }
}

impl VulkanDevice {
    pub unsafe fn find_memory_type(
        &self, type_filter: u32, properties: ash::vk::MemoryPropertyFlags,
    ) -> Option<u32> {
