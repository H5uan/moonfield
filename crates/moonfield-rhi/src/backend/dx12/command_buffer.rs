...
use crate::{
    CommandBuffer, RenderPassDescriptor, RhiError, SwapchainImage, types::*,
};
...
impl Dx12CommandBuffer {
    pub fn new(
        device: &ID3D12Device, command_allocator: &ID3D12CommandAllocator,
        ...
        unsafe {
            let command_list = device
                ...
                .map_err(|e| {
                    RhiError::CommandBufferAllocationFailed(format!(
    ...
    pub fn get_command_list(
        ...
        std::sync::RwLockReadGuard<Option<ID3D12GraphicsCommandList>>,
        RhiError,
    > {
        self.command_list.read().map_err(|_| {
            RhiError::CommandBufferAllocationFailed(
                "Failed to lock command list".to_string(),
            )
        })
    }

    pub fn get_command_list_mut(
        &self,
    ) -> Result<
        std::sync::RwLockWriteGuard<Option<ID3D12GraphicsCommandList>>,
        RhiError,
    > {
        self.command_list.write().map_err(|_| {
            RhiError::CommandBufferAllocationFailed(
                "Failed to lock command list".to_string(),
            )
        })
...
impl CommandBuffer for Dx12CommandBuffer {
    fn begin(&self) -> StdResult<(), RhiError> {
        ...
        if *is_recording {
            return Err(RhiError::CommandBufferAllocationFailed(
    ...
    fn end(&self) -> StdResult<(), RhiError> {
        ...
        unsafe {
            ...
            cmd_list_guard.as_ref().unwrap().Close().map_err(|e| {
                RhiError::CommandBufferAllocationFailed(format!(
    ...
    fn begin_render_pass(
        &self, _desc: &RenderPassDescriptor, _image: &SwapchainImage,
    ) {
    ...
    fn bind_pipeline(&self, pipeline: &dyn crate::Pipeline) {
        unsafe {
            ...
            if let Some(ref command_list) = *cmd_list_guard {
                ...
                if let Some(dx12_pipeline) =
                    pipeline_any.downcast_ref::<super::pipeline::Dx12Pipeline>()
                {
                    command_list
                        .SetPipelineState(&dx12_pipeline.pipeline_state);
                }
            }
        }
    }
    ...
    fn bind_vertex_buffer(&self, buffer: &dyn crate::Buffer) {
        unsafe {
            ...
            if let Some(ref command_list) = *cmd_list_guard {
                ...
                if let Some(dx12_buffer) =
                    buffer_any.downcast_ref::<super::buffer::Dx12Buffer>()
                {
                    let gpu_virtual_address =
                        dx12_buffer.buffer.GetGPUVirtualAddress();
