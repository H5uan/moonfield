...
use windows::{
    Win32::Foundation::*, Win32::Graphics::Direct3D12::*,
    Win32::Graphics::Dxgi::*, core::*,
};

use crate::{
    Format, GraphicsPipelineDescriptor, Pipeline, RhiError, VertexFormat,
    VertexInputDescriptor, VertexInputRate, types::*,
};
...
impl Dx12Pipeline {
    pub fn new(
        device: &super::device::Dx12Device, desc: &GraphicsPipelineDescriptor,
    ) -> StdResult<Self, RhiError> {
        unsafe {
            // Get shader modules
            let vertex_shader_any = desc.vertex_shader.as_any();
            let vertex_shader = vertex_shader_any
                .downcast_ref::<super::shader_module::Dx12ShaderModule>()
                .ok_or_else(|| {
                    RhiError::PipelineCreationFailed(
                        "Invalid vertex shader module type".to_string(),
                    )
                })?;

            let fragment_shader_any = desc.fragment_shader.as_any();
            let fragment_shader = fragment_shader_any
                .downcast_ref::<super::shader_module::Dx12ShaderModule>()
                .ok_or_else(|| {
                    RhiError::PipelineCreationFailed(
                        "Invalid fragment shader module type".to_string(),
                    )
                })?;
            ...
            let input_layout =
                convert_vertex_input_to_d3d12_layout(&desc.vertex_input)?;
            ...
            let pso_desc = D3D12_GRAPHICS_PIPELINE_STATE_DESC {
                ...
                VS: D3D12_SHADER_BYTECODE {
                    pShaderBytecode: vertex_shader.get_bytecode().as_ptr()
                        as *const std::ffi::c_void,
                    BytecodeLength: vertex_shader.get_bytecode().len(),
                },
                PS: D3D12_SHADER_BYTECODE {
                    pShaderBytecode: fragment_shader.get_bytecode().as_ptr()
                        as *const std::ffi::c_void,
                ...
                BlendState: D3D12_BLEND_DESC {
                    ...
                    RenderTarget: [D3D12_RENDER_TARGET_BLEND_DESC {
                        ...
                        RenderTargetWriteMask: D3D12_COLOR_WRITE_ENABLE_ALL.0
                            as u8,
                    }; 8],
                },
                SampleMask: u32::MAX,
            ...
            Ok(Dx12Pipeline { pipeline_state, input_layout, root_signature })
        }
    }
}

unsafe fn create_default_root_signature(
    device: &ID3D12Device,
) -> StdResult<ID3D12RootSignature, RhiError> {
    ...
    if hr.is_err() {
        return Err(RhiError::PipelineCreationFailed(
            "Failed to serialize root signature".to_string(),
        ));
    }

    let signature_blob = signature_blob.ok_or_else(|| {
        RhiError::PipelineCreationFailed(
            "Failed to create root signature blob".to_string(),
        )
    })?;
    ...
    let root_signature =
        device.CreateRootSignature(0, signature_data).map_err(|e| {
            RhiError::PipelineCreationFailed(format!(
...
fn convert_vertex_input_to_d3d12_layout(
    vertex_input: &VertexInputDescriptor,
) -> StdResult<Vec<D3D12_INPUT_ELEMENT_DESC>, RhiError> {
    ...
    for attr in &vertex_input.attributes {
        ...
        elements.push(D3D12_INPUT_ELEMENT_DESC {
            ...
            InputSlotClass: if vertex_input
                .bindings
            ...
            InstanceDataStepRate: if vertex_input
                .bindings
                .get(attr.binding as usize)
                .map_or(VertexInputRate::Vertex, |binding| binding.input_rate)
                == VertexInputRate::Instance
            {
                1
            } else {
