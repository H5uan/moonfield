...
impl Dx12Device {
    pub fn new(d3d12_device: &ID3D12Device) -> StdResult<Self, RhiError> {
        ...
        let queue = unsafe {
            ...
            let queue: ID3D12CommandQueue =
                d3d12_device.CreateCommandQueue(&queue_desc).map_err(|e| {
                    RhiError::DeviceCreationFailed(format!(
        ...
        Ok(Dx12Device { device: d3d12_device.clone(), queue })
    }
...
impl Device for Dx12Device {
    ...
    ) -> StdResult<Arc<dyn Swapchain>, RhiError> {
        // Extract the surface as Dx12Surface to get the HWND
        let surface_any = desc.surface.as_any();
        let dx12_surface = surface_any
            .downcast_ref::<super::surface::Dx12Surface>()
            .ok_or_else(|| {
                RhiError::SwapchainCreationFailed(
                    "Invalid surface type for DX12".to_string(),
                )
            })?;

        let dx12_swapchain =
            super::swapchain::Dx12Swapchain::new(self, dx12_surface, desc)?;
        Ok(Arc::new(dx12_swapchain) as Arc<dyn Swapchain>)
    }

    fn create_shader_module(
        &self, desc: &ShaderModuleDescriptor,
    ) -> StdResult<Arc<dyn ShaderModule>, RhiError> {
    ...
    ) -> StdResult<Arc<dyn Pipeline>, RhiError> {
    ...
    ) -> StdResult<Arc<dyn Buffer>, RhiError> {
        let dx12_buffer = super::buffer::Dx12Buffer::new(self, desc)?;
        Ok(Arc::new(dx12_buffer) as Arc<dyn Buffer>)
    }

    fn create_command_pool(
        &self, _swapchain: &Arc<dyn Swapchain>,
    ) -> StdResult<Arc<dyn CommandPool>, RhiError> {
        let dx12_command_pool =
            super::command_pool::Dx12CommandPool::new(self)?;
