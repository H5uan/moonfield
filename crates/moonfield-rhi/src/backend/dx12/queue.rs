...
use crate::{CommandBuffer, Queue, RhiError, types::*};
...
impl Dx12Queue {
    pub fn new(queue: &ID3D12CommandQueue) -> Self {
        Dx12Queue { queue: queue.clone() }
    }
}

impl Queue for Dx12Queue {
    fn submit(
        &self, _command_buffers: &[Arc<dyn CommandBuffer>],
    ...
    fn wait_idle(&self) -> StdResult<(), RhiError> {
        unsafe {
            ...
            // Signal the fence
            self.queue.Signal(&fence, 1).map_err(|e| {
                RhiError::SubmitFailed(format!("Failed to signal fence: {}", e))
            })?;
