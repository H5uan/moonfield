...
use crate::{Extent2D, Format, RhiError, Swapchain, SwapchainImage, types::*};
...
impl Dx12Swapchain {
    pub fn new(
        ...
        surface: &super::surface::Dx12Surface, desc: &SwapchainDescriptor,
...
unsafe fn create_dxgi_swapchain(
    device: &super::device::Dx12Device, surface: &super::surface::Dx12Surface,
    ...
    let adapter = {
        ...
        if hr.is_err() {
            return Err(RhiError::SwapchainCreationFailed(
                "Failed to get adapter from device".to_string(),
            ));
        }
        adapter.unwrap()
    };
    ...
    let factory: IDXGIFactory4 = {
        ...
        if hr.is_err() {
            return Err(RhiError::SwapchainCreationFailed(
                "Failed to get factory from adapter".to_string(),
            ));
        }

        factory.unwrap().cast::<IDXGIFactory4>().map_err(|e| {
            RhiError::SwapchainCreationFailed(format!(
                "Failed to get IDXGIFactory4: {}",
                e
            ))
        })?
    };
    ...
    let swapchain: IDXGISwapChain3 =
        swapchain.cast::<IDXGISwapChain3>().map_err(|e| {
            RhiError::SwapchainCreationFailed(format!(
                "Failed to cast swapchain: {}",
                e
            ))
        })?;
...
impl Swapchain for Dx12Swapchain {
    ...
    fn present(&self, _image: SwapchainImage) -> StdResult<(), RhiError> {
        unsafe {
            ...
            if hr.is_err() {
                return Err(RhiError::PresentFailed(
                    "Failed to present swapchain".to_string(),
                ));
            }
        }
        Ok(())
