...
impl Device for MetalDevice {
    fn create_swapchain(
        &self, desc: &SwapchainDescriptor,
    ) -> Result<Arc<dyn Swapchain>, RhiError> {
        tracing::debug!(
            "Creating Metal swapchain with format: {:?}, extent: {:?}",
            desc.format,
            desc.extent
        );
        unsafe {
    ...
    fn create_shader_module(
        &self, desc: &ShaderModuleDescriptor,
    ) -> Result<Arc<dyn ShaderModule>, RhiError> {
        tracing::debug!(
            "Creating Metal shader module for stage: {:?}",
            desc.stage
        );
        unsafe {
            let source = std::str::from_utf8(desc.code).map_err(|e| {
                tracing::error!("Invalid UTF-8 in shader source: {}", e);
                RhiError::ShaderCompilationFailed(
                    ShaderCompilationError::InvalidShaderCode(format!(
                        "Invalid UTF-8 in shader source: {}",
                        e
                    )),
                )
            })?;
            ...
            let library = self
                ...
                .map_err(|e| {
                    ...
                    RhiError::ShaderCompilationFailed(
                        ShaderCompilationError::CompilationError(format!(
                            "Failed to create Metal shader library: {:?}",
                            e
                        )),
                    )
            ...
            tracing::info!(
                "Metal shader module created successfully for stage: {:?}",
                desc.stage
            );
            Ok(Arc::new(MetalShaderModule { library, stage: desc.stage }))
        }
    }

    fn create_pipeline(
        &self, desc: &GraphicsPipelineDescriptor,
    ) -> Result<Arc<dyn Pipeline>, RhiError> {
        ...
        unsafe {
            ...
            let vs_func = vs
                .library
                .newFunctionWithName(&vs_func_name)
                .ok_or_else(|| {
                    tracing::error!("Failed to get vertex shader function");
                    RhiError::PipelineCreationFailed(
                        "Failed to get vertex shader function".to_string(),
                    )
                })?;
            ...
            let fs_func = fs
                .library
                .newFunctionWithName(&fs_func_name)
                .ok_or_else(|| {
                    tracing::error!("Failed to get fragment shader function");
                    RhiError::PipelineCreationFailed(
                        "Failed to get fragment shader function".to_string(),
                    )
                })?;
            ...
            let pipeline_state = self
                ...
                .map_err(|e| {
                    ...
                    RhiError::PipelineCreationFailed(format!(
                        "Failed to create Metal render pipeline state: {:?}",
                        e
                    ))
                })?;
            ...
            Ok(Arc::new(MetalPipeline { pipeline_state }))
        }
    }

    fn create_buffer(
        &self, desc: &BufferDescriptor,
    ) -> Result<Arc<dyn Buffer>, RhiError> {
        tracing::debug!(
            "Creating Metal buffer with size: {}, memory location: {:?}",
            desc.size,
        ...
        unsafe {
            ...
            let buffer = self
                ...
                .map_err(|_| {
                    tracing::error!(
                        "Failed to create Metal buffer with size: {}",
                        desc.size
                    );
                    RhiError::BufferCreationFailed(format!(
                        "Failed to create Metal buffer with size: {}",
                        desc.size
                    ))
                })?;
            ...
            tracing::info!(
                "Metal buffer created successfully with size: {}",
                desc.size
            );
    ...
    fn create_command_pool(
        &self, swapchain: &Arc<dyn Swapchain>,
    ) -> Result<Arc<dyn CommandPool>, RhiError> {
        tracing::debug!("Creating Metal command pool");
        tracing::info!("Metal command pool created successfully");
        Ok(Arc::new(MetalCommandPool { queue: self.queue.clone() }))
    }

    fn get_queue(&self) -> Arc<dyn Queue> {
        Arc::new(MetalQueue { queue: self.queue.clone() })
    }
}
    ...
    fn acquire_next_image(&self) -> Result<SwapchainImage, RhiError> {
        ...
        unsafe {
            let drawable = self.layer.nextDrawable().ok_or_else(|| {
                tracing::error!(
                    "Failed to acquire next drawable from Metal layer"
                );
                RhiError::AcquireImageFailed(
                    "Failed to acquire next drawable from Metal layer"
                        .to_string(),
                )
            })?;
            ...
            tracing::debug!(
                "Successfully acquired next image from Metal swapchain"
            );
            Ok(SwapchainImage {
                index: 0,
...
impl CommandPool for MetalCommandPool {
    fn allocate_command_buffer(
        ...
        unsafe {
            let command_buffer =
                self.queue.commandBuffer().ok_or_else(|| {
                    tracing::error!("Failed to allocate Metal command buffer");
                    RhiError::CommandBufferAllocationFailed(
                        "Failed to allocate Metal command buffer".to_string(),
                    )
                })?;
...
impl CommandBuffer for MetalCommandBuffer {
    ...
    fn begin_render_pass(
        &self, desc: &RenderPassDescriptor, image: &SwapchainImage,
    ) {
        unsafe {
    ...
    fn bind_pipeline(&self, pipeline: &dyn Pipeline) {
        ...
        let metal_pipeline =
            pipeline_any.downcast_ref::<MetalPipeline>().unwrap();
...
fn draw(
    &self, vertex_count: u32, instance_count: u32, first_vertex: u32,
    first_instance: u32,
) {
    unsafe {
        if let Some(encoder) = &self.current_encoder {
...
impl Queue for MetalQueue {
    fn submit(
        ...
        _wait_semaphore: Option<u64>, _signal_semaphore: Option<u64>,
    ) -> Result<(), RhiError> {
        tracing::debug!(
            "Submitting {} command buffers to Metal queue",
            command_buffers.len()
        );
        ...
        tracing::debug!(
            "Successfully submitted {} command buffers to Metal queue",
            command_buffers.len()
        );
        Ok(())
    }
