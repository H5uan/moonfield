...
/// Monotonic version assigned whenever an asset is inserted or mutably
/// accessed through [`Assets::get_mut`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct AssetRevision(u64);

impl AssetRevision {
    /// The raw monotonic revision value.
    pub fn get(self) -> u64 {
        self.0
    }
}
...
impl<T: 'static> Assets<T> {
    ...
    pub fn get_mut(&mut self, handle: &Handle<T>) -> Option<&mut T> {
        let index = handle.id.index as usize;
        let slot = self.slots.get(index)?;
        if slot.generation != handle.id.generation || slot.asset.is_none() {
            return None;
        }
        let revision = self.take_revision();
    ...
    /// The current revision for a live handle.
    pub fn revision(&self, handle: &Handle<T>) -> Option<AssetRevision> {
        let slot = self.slots.get(handle.id.index as usize)?;
        (slot.generation == handle.id.generation && slot.asset.is_some()).then_some(slot.revision)
    }
    ...
    /// Iterate `(handle, revision, asset)` triples for all live assets.
    pub fn iter_with_revision(&self) -> impl Iterator<Item = (Handle<T>, AssetRevision, &T)> {
        self.slots.iter().enumerate().filter_map(|(index, slot)| {
            slot.asset.as_ref().map(|asset| {
                (
                    Handle::from_id(AssetId {
    ...
    fn take_revision(&mut self) -> AssetRevision {
        let revision = AssetRevision(self.next_revision);
        self.next_revision = self
            .next_revision
            .checked_add(1)
            .expect("asset revision counter overflowed");
        revision
    }
}
...
mod tests {
    ...
    fn test_get_mut_edits_in_place() {
        ...
        let h = assets.add(1);
        let before = assets.revision(&h).unwrap();
        *assets.get_mut(&h).unwrap() = 42;
        assert_eq!(assets.get(&h), Some(&42));
        assert!(assets.revision(&h).unwrap() > before);
    }

    #[test]
    fn test_each_asset_mutation_gets_a_new_revision() {
        let mut assets = Assets::<u32>::default();
        let first = assets.add(1);
        let second = assets.add(2);
        ...
        let changed_revision = assets.revision(&first).unwrap();
        assert!(changed_revision > second_revision);

        let iterated: Vec<_> = assets
            .iter_with_revision()
            .map(|(handle, revision, value)| (handle, revision, *value))
            .collect();
        assert!(iterated.contains(&(first, changed_revision, 1)));
        assert!(iterated.contains(&(second, second_revision, 2)));
    }
