...
///
/// Drop order is the reverse of first-insertion order (LIFO), mirroring Rust
/// struct-field drop: resources that were created from an earlier resource
/// (e.g. GPU objects created from a `RenderDevice`) are destroyed before the
/// resource they depend on. Vulkan wrapper types hold raw `ash` handles, so
...
impl Resources {
    pub fn insert<R: Resource>(&mut self, res: R) {
        ...
        self.data.insert(id, RefCell::new(Box::new(res)));
    }
...
impl Drop for Resources {
    fn drop(&mut self) {
        // LIFO teardown; `remove` skips keys already taken out at runtime.
        for id in std::mem::take(&mut self.order).into_iter().rev() {
            if let Some(cell) = self.data.remove(&id) {
                drop(cell);
            }
        }
    }
