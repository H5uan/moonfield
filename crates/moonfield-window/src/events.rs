...
//! (close requests, resize, focus) rather than gameplay input. Delivery uses
//! the message channel: the windowing backend (e.g. `moonfield-winit`)
//! registers `WindowEventKind` via `App::add_message` and writes into the
//! `Messages<WindowEventKind>` resource as events arrive; consumers read
//! them with a `MessageReader<WindowEventKind>` system param (two-frame
//! retention — see `moonfield_ecs::Messages`).
//!
