...
use crate::asset::common::{
    MaterialAsset, MaterialProperties, MeshAsset, ShaderAsset, ShaderType,
    TextureAsset, TextureFormat, VertexAttribute, VertexFormat, VertexLayout,
};
...
impl AssetLoader<TextureAsset> for TextureLoader {
    fn load(
        &self, path: &Path,
    ) -> Result<TextureAsset, Box<dyn std::error::Error>> {
        ...
        Ok(TextureAsset { width, height, format, data: file_data })
    }
}
...
impl AssetLoader<MeshAsset> for MeshLoader {
    fn load(
        &self, path: &Path,
    ) -> Result<MeshAsset, Box<dyn std::error::Error>> {
        ...
        match path.extension().and_then(|ext| ext.to_str()) {
            Some("obj") => parse_obj_mesh(&file_content),
            Some("gltf") | Some("glb") => parse_gltf_mesh(&file_content),
            _ => {
...
fn parse_obj_mesh(
    ...
    Ok(MeshAsset { vertices, indices, layout })
}
...
fn create_default_cube_mesh() -> MeshAsset {
    ...
    MeshAsset { vertices, indices, layout }
}
...
impl AssetLoader<ShaderAsset> for ShaderLoader {
    fn load(
        &self, path: &Path,
    ) -> Result<ShaderAsset, Box<dyn std::error::Error>> {
        let file_data = fs::read(path)?;

        // Determine shader type based on file extension
        let shader_type = match path.extension().and_then(|ext| ext.to_str()) {
            Some("vert") | Some("vertex") | Some("vs") | Some("vert.spv")
            | Some("vs.spv") => ShaderType::Vertex,
            Some("frag") | Some("fragment") | Some("fs") | Some("frag.spv")
            | Some("fs.spv") => ShaderType::Fragment,
            Some("comp") | Some("compute") | Some("cs") | Some("comp.spv")
            | Some("cs.spv") => ShaderType::Compute,
            Some("geom") | Some("geometry") | Some("geom.spv") => {
                ShaderType::Geometry
            }
        ...
        Ok(ShaderAsset { source: file_data, shader_type, entry_point })
    }
}
