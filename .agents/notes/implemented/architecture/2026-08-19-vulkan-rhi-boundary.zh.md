...
`moonfield-render` 是唯一链接 `ash` 的 crate,所有 Vulkan 专属代码位于 `src/vulkan/`(device、swapchain、pipeline、command、sync、offscreen、window_target、shader)。它暴露的表面使用自己的词汇:
...
- `cargo test -p moonfield-render --test headless_triangle` 在 lavapipe 上无头运行;Windows/macOS 无 Vulkan 驱动时优雅跳过。
