...
- **ECS→winit 每帧差分一次**:`App::update` 之后,`sync_windows`(`windows.rs`)把实时 `Window` 字段与 `CachedWindow` 组件做 diff,`diff_window` 返回 `WindowDiff`(标题、光标模式),由后端应用到原生窗口。这是 Bevy 的 `CachedWindow` 模式(无变更检测)。
...
- 退出策略镜像 Godot 的 `auto_accept_quit`:`CloseRequested` 默认退出;`WindowControl::set_auto_exit_on_close(false)` 接管控制,之后 `WindowControl::request_exit()` 退出。
