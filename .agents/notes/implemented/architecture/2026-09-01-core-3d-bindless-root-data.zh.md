...
- 管线的 push-constant 范围从 80 B 缩到 8 B（`ROOT_POINTER_SIZE`，单个 `GpuPtr`）；`DrawMesh` 经 bump host 指针写入 `DrawData`，并把设备地址作为根推送。顶点/索引缓冲保留经典绑定——本里程碑只改根数据。
...
- 用字面 `push_data`（descriptor heap 的 root bank）取代 push constant 里的根指针：`vkCmdPushDataEXT` 的 GPU 侧消费端尚未接线——`command_push_data` 只验证录制，且 Slang 的入口 `Ptr` 参数降级仅对 push constant 有验证。采纳它需先做未经证实的 RHI/shader 工作，故推迟。
...
- 在纯 heap 管线（无描述符集布局）上，push constant 用量从每 draw 80 B 降到 8 B。
