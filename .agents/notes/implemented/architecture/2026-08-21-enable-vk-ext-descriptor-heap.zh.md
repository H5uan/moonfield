...
笔记](2026-08-21-vulkan-1-4-via-ash-git-master.zh.md)),且目标平台现在仅限
较新的 NVIDIA 驱动。启用该扩展是直接可写的 descriptor heap 模型的先决
...
- **仅在枚举到时启用扩展、保留旧路径**:否决——RHI 只面向 NVIDIA 平台;
  静默的双代码路径正是目标平台规则要排除的分叉。
...
- `cargo test` 在 CI 上保持绿色:不支持的驱动打印跳过原因,支持的驱动跑
  完整套件(本机 NVIDIA 驱动会真正跑全部 GPU 测试)。
