...
- `DeviceExtensionFunctions` —— 聚合所有 loader 的结构体,在
  `Device::from_physical_device` 中只构建一次,内联存放在 `DeviceShared`
  (经 `Arc` 共享的设备拆卸状态)里(与 wgpu 的 `DeviceExtensionFunctions`
  在 `Arc<DeviceShared>` 内的形态一致)。
- `CommandPool`/`CommandBuffer` 经由各自的 `DeviceContext`(每个 GPU 对象
  都持有的 `Arc<DeviceShared>` 句柄)访问这张表。loader 是函数指针表;
  克隆 context 不复制任何表。
- 调用点直接字段访问 loader:
  `self.ctx.extension_fns().extended_dynamic_state3.cmd_set_*` —— 字段
  访问,不再有逐扩展 getter。`CommandBuffer` 从不复制表,通过共享的 `Arc`
  解引用。
...
- **只把整张表存在 `CommandBuffer` 上。** loader 是设备作用域的概念;从设备
  共享保持单一真源,也让其他设备消费者(如未来的 GPU-driven 录制器)能拿到
  同一张表。

## Consequences

- 新增扩展 loader 现在只需:在 `DeviceExtensionFunctions` 加一个字段、在
  `from_physical_device` 加载一次 —— 完成。
- `CommandPool`/`CommandBuffer` 经由 `DeviceContext` 共享表;热路径 draw
  只通过共享指针解引用一次,与本地字段的间接调用成本相同。
