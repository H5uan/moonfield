...
`moonfield-render` is the only crate that links `ash`, and all Vulkan-specific
code lives in `src/vulkan/` (device, swapchain, pipeline, command, sync,
offscreen, window_target, shader). The surface it exposes is its own vocabulary:

- Public resource descriptions — `Format`, `BufferUsage`, `VertexBufferLayout` —
  are declared in `src/types.rs`, never raw `ash` types.
...
- Shaders: the backend compiles Slang→SPIR-V at runtime
  ...
  `include_bytes!`.
- `cargo test -p moonfield-render --test headless_triangle` runs headless on
