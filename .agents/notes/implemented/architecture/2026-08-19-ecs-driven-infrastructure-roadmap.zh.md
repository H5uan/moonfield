...
- 已知欠债(已记录,未排期):渲染世界/extract 分离、异步 `AssetServer` 加任务池、编辑器视口中的真实 splat 光栅化(今天是 AABB 占位)、多线程执行器、固定更新 schedule(`Time<Fixed>`)、`With`/`Without` 查询过滤器、完整 observer、完整反射(`DynamicStruct`)、强/弱句柄引用计数,以及音频/物理/序列化/网络层。
