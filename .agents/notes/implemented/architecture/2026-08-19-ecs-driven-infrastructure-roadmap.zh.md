...
8. **资产,同步优先** —— 零依赖的 `Assets<T>` slot-map 存储与 index+generation 的 `Handle<T>`;`SplatCloud` 是第一个资产,由调用方同步加载;视口渲染是 AABB 占位;训练状态留在 `World` 之外。
...
- 已知欠债(已记录,未排期):渲染世界/extract 分离、异步 `AssetServer` 加任务池、编辑器视口中的真实 splat 光栅化(今天是 AABB 占位)、多线程执行器、固定更新 schedule(`Time<Fixed>`)、`Events<T>`(用以替换手写的 `WindowEvents`/`InputState` 逐帧清空模式的候选)、`With`/`Without` 查询过滤器、完整 observer、完整反射(`DynamicStruct`)、强/弱句柄引用计数,以及音频/物理/序列化/网络层。
