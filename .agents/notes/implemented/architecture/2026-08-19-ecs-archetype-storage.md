...
`moonfield-ecs::world::World` stores entities in **archetypes**: an entity's
...
- `entities` map holds `EntityId` → (archetype index, row index); zero is
  reserved for entities with no components, flushed in batch by `World::flush`.
...
- `World` is the unified world type; the temporary migration name `World2` was
  retired once the sparse-set implementation was removed.
