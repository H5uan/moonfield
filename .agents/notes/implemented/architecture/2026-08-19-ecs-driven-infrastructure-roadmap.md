...
8. **Assets, sync first** — the zero-dependency `Assets<T>` slot-map store
   ...
   loaded synchronously by the caller; viewport rendering is an AABB
   placeholder; training state stays outside the `World`.
...
- Known debts, recorded but not scheduled: the render-world/extract split, an
  ...
  fixed-update schedule (`Time<Fixed>`), `Events<T>` (candidate to replace
  the hand-rolled `WindowEvents`/`InputState` frame-clearing pattern),
