...
`moonfield-ecs::world::World` 以 **archetype** 存储实体:实体的组件集合决定它所在的 archetype,每个 archetype 用扁平紧凑的切片存放组件(`archetype.rs`)。配套部件:

- `bundle_to_archetype` 缓存 `TypeId` bundle → archetype 查找;`insert_edges` / `remove_edges` 缓存 archetype 间的迁移边。
- `entities` 表保存 `EntityId` → (archetype 索引, 行索引);0 号 archetype 保留给无组件实体,由 `World::flush` 批量分配。
...
- `World` 是统一后的世界类型名;迁移期临时名 `World2` 已随稀疏集实现的移除而退役。
