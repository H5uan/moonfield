...
target now is recent desktop drivers that expose the extension — recorded
at the time as NVIDIA-only, but AMD's proprietary Windows driver exposes
it too (verified on an RX 9070 XT, driver 32.0.23034.4). Enabling the
extension is the
...
- No fallback: a driver that does not enumerate the extension fails
  `vkCreateDevice`. The RHI targets drivers exposing the extension (current
  NVIDIA and AMD proprietary) and ships no code path that works without the
  heap.
...
- **Enable the extension only when enumerated, keep a legacy path**:
  rejected — the RHI targets only descriptor-heap-capable platforms; a
  silent secondary code
...
- `cargo test` stays green on CI: unsupported drivers print the skip
  reason; supported ones run the full suite (the local AMD driver runs
  all GPU tests).
