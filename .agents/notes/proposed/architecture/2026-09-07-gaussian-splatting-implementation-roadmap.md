...
- **Gradient accumulation is buffer float32 atomics** (`VK_EXT_shader_atomic_float`; the dev machine's T1000 exposes `shaderBufferFloat32AtomicAdd`). Kernels zero the gradient buffers each step. Training results are not bit-reproducible: training-loop regression criteria are statistical (loss/PSNR thresholds), while the viewing path stays exact.
- **One differentiable tile-based forward serves viewing and training.** It writes per-tile sorted Gaussian lists and per-pixel transmittance to buffers; `render-feature` records it as a render-graph node, `moonfield-ml` records the same dispatches inside each training step. Viewing pays the artifact-write bandwidth.
...
- **M1 — the `moonfield-ml` training loop on the public RHI API.** `Trainer::run` over `submit_and_wait`, the Adam kernel (`assets/shaders/ml/adam.slang`), and a minimal `TrainingMethod` refitting the `gaussian_fit` target with an atomicAdd backward — the probe that Slang emits buffer float atomics on this driver. The RHI's optional-extension table gains `VK_EXT_shader_atomic_float`.
...
- **M3 — the tile-based forward.** Projection, tile bucketing, radix sort, alpha blending, with the artifacts in buffers. `render-feature` records it as a render-graph node (offscreen test first, then the editor viewport); `moonfield-ml` records the same forward per step.
...
- Slang-to-SPIR-V float-atomic emission on this driver is unproven until M1's backward runs; failure reopens warp reduction.
