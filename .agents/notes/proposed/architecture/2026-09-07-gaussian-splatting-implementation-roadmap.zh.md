...
- **梯度累积用 buffer float32 原子操作**(`VK_EXT_shader_atomic_float`;开发机的 T1000 暴露 `shaderBufferFloat32AtomicAdd`)。内核每步清零梯度缓冲。训练结果不再逐位可复现:训练回路的回归判据改为统计值(loss/PSNR 阈值),观看路径保持精确。
- **一套可微的 tile 前向同时服务观看与训练。** 它把每个 tile 排序后的高斯列表与每像素透射率写入缓冲;`render-feature` 把它录成 render-graph 节点,`moonfield-ml` 在每个训练步里录同一组分发。观看路径为中间产物的写出付带宽。
...
- **M1 —— `moonfield-ml` 训练回路跑在 RHI 公共 API 上。** `Trainer::run` 走 `submit_and_wait`,Adam 内核(`assets/shaders/ml/adam.slang`),一个最小 `TrainingMethod` 以 atomicAdd 反向重拟合 `gaussian_fit` 目标——即 Slang 在本驱动上发出 buffer float 原子操作的探针。RHI 的可选扩展表加入 `VK_EXT_shader_atomic_float`。
...
- **M3 —— tile 前向。** 投影、tile 分桶、radix sort、alpha 混合,中间产物落缓冲。`render-feature` 将其录成 render-graph 节点(先离屏测试,后编辑器视口);`moonfield-ml` 每步录同一前向。
...
- Slang 到 SPIR-V 的 float 原子操作发射在 M1 反向跑通之前未经验证;失败则 warp 归约重开。
